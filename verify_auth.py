...
def test_playlist_permissions(access_token: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                "   ℹ️  If playlist creation fails, ensure your OAuth app has the right scopes"
            )
            return True
...
def test_video_details_access(access_token: str) -> bool:
    ...
    params = {"part": "contentDetails,statistics", "id": "dQw4w9WgXcQ"}
    headers = {"Authorization": f"Bearer {access_token}"}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("items"):
                ...
                print(
                    f"   ✅ Can get video details (duration: {duration}, views: {views})"
                )
                return True
            else:
